angular.module("app.admin.orders", ["app.admin.orders.services","app.admin.restaurant.manage.orders.services"])
    .controller("AdminOrderController", function($scope, $modal, $timeout, adminOrderResource,
                                                 globalHandleUI, $stateParams) {
        $scope.init = function() {
            ...
            $scope.page = 1,
                ...
                $scope.loadMoreOrders = function() {
                    var params = $scope.initLazyLoad($scope.page, $scope.records),
                        data = [];
                    $scope.isScrollEnd && adminOrderResource.get(params).success(function(payload) {
                        data = payload.data.orders, isLoad = !1, $scope.orders = $scope.orders.concat(data)
                    })
                },
                ...
                adminOrderResource.get("").success(function(payload) {
                ...
                status && (params = "?status=" + status),
                    adminOrderResource.get(params).success(function(payload) {
                        console.log(payload);
            ...
            }, $timeout($scope.handleTableScroll, 200),
                    ...
                    controller: function($scope, $modalInstance, adminOrdersResource, itemToProcess) {
                        $scope.init = function() {
                            ...
                            $scope.tempItem = angular.copy(item),
                                ...
                                delete $scope.tempItem.customer,
                                delete $scope.tempItem.orderDetails,
                                $scope.acceptLoading = !0,
                                setTimeout(function() {
                                    adminOrderResource.addOrUpdate($scope.tempItem).success(function() {
                                        $modalInstance.close()
                                    })
                                }, 300)
