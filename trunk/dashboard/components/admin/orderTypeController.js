angular.module("app.admin.restaurant.manage.restaurant.ordertype", ["ui.calendar"])
    .controller("AdminOrderTypeController", function($scope, $state, $stateParams, $rootScope,
                                                     $compile, uiCalendarConfig, handleCalendarHours, adminTimeResource, adminInfoResource) {
        $scope.events = [], $scope.options = {
            dineIn: {
                title: "Dine-In",
                color: "#007802",
                textColor: "white"
        ...
        }, $scope.isLoadingEvents = !0, $scope.restaurantID = $stateParams.restaurantId,
            $scope.init = function() {
                $scope.datetimes = handleCalendarHours.formatDatetime, $scope.isAccept = {
                    delivery: !1,
                    takeout: !1
                };
                var convertHoursToEvents = function(hours) {
                    for (var events = [], i = 0; i < hours.dineInHours.length; i++) events.push({
                ...
                setTimeout(function() {
                    adminInfoResource.get($scope.restaurantID).success(function(payload) {
                        $scope.hours = {
            ...
            $scope.initCalendar = function() {
                ...
                }, $scope.eventSources = [$scope.events], $scope.addEvents = function() {
                    return {
            ...
            }, $scope.initCalendar(), $scope.initEndTime = function() {
            return {
            ...
            var objectModel = {};
            objectModel = handleCalendarHours.synObjectOnAPI($scope.events),
                objectModel.acceptDelivery = $scope.isAccept.delivery,
                objectModel.acceptTakeOut = $scope.isAccept.takeout,
                objectModel.timeZoneId= "Asia/Kolkata",
                adminTimeResource.putTime($scope.restaurantID, objectModel).success(function() {
