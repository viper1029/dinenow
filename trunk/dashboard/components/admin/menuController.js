angular.module("app.admin.restaurant.manage.menu.menus", [])
    .controller("AdminMenusController", function($scope, $filter, $state, $stateParams, $modal,
                                                 ngTableConfig, adminSubmenusResource, adminMenusResource, globalHandleUI) {
        globalHandleUI.handleToggleLeftBar(),
            $scope.isCollapsed = !1,
            $scope.menuList = [], $scope.tempSubmenuTree = [],
            $scope.submenuTree = [],
            $scope.menuTree = {
                name: "",
                description: "",
            ...
            $scope.getSubMenuList = function() {
                var restaurantID = $stateParams.restaurantId;
                adminSubmenusResource.get(restaurantID).success(function(payload) {
                    $scope.submenuTree = payload.data.submenus,
                        $scope.tempSubmenuTree = angular.copy($scope.submenuTree)
                }).error(function() {})
            },
            $scope.getSubMenuByID = function(param) {
                $scope.submenuTree = [], setTimeout(function() {
                    adminSubmenusResource.getByID(param).success(function(payload) {
                        $scope.submenuTree.push(payload.data.submenus),
                            $scope.tempSubmenuTree = angular.copy($scope.submenuTree)
                    })
                }, 250)
            },
            $scope.init = function() {
                ...
                $scope.isLoadingList = !0, setTimeout(function() {
                    $scope.getMenuList(), $scope.isLoadingList = !1
            ...
            $scope.syncObject = function(obj) {
                ...
                return tempObj.submenus = temp.submenus, tempObj.restaurantId = $stateParams.restaurantId || "", tempObj
            },
                ...
                $scope.isLoadingSave = !0, setTimeout(function() {
                    adminMenusResource.addOrUpdate(obj).success(function() {
                        $state.go("app.admin.restaurant.manage.menu.menus.list", {}, {
                            reload: !0
                        }), alertify.success("Created successfully"), $scope.isLoadingSave = !1
                    }).error(function() {
                        alertify.error("Oops! Please try again")
                    })
                }, 500)
            ...
            $scope.editMenu = function(item) {
                item.editLoading = !0, setTimeout(function() {
                    $state.go("app.admin.restaurant.manage.menu.menus.edit"), adminMenusResource.getByID(item).success(function(payload) {
                        if ($scope.menuTree = payload.data.menu, $scope.submenuTree = angular.copy($scope.tempSubmenuTree), $scope.menuTree.submenus.length > 0)
