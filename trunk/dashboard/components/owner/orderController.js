angular.module("app.owner.orders", ["common.directives", "app.owner.orders.services"])
    .controller("OrdersController", function($scope, $modal, $timeout, ordersResource, globalHandleUI, $cookieStore) {
        $scope.restaurantID= $cookieStore.get("authRestaurantID"),
            console.log($scope.restaurantID),
            $scope.init = function() {

                $scope.orders = [], $scope.page = 1,
                    $scope.records = 10,
                    $scope.isScrollEnd = !0,
                    $scope.daterange = {
                    ...
                    var params = $scope.initLazyLoad($scope.page, $scope.records),
                        data = [];
                    $scope.isScrollEnd && ordersResource.get($scope.restaurantID, params).success(function(payload) {
                    ...
                    if ($scope.status = {
                        ...
                        for (var i = 0; i < listOrder.length; i++)
                            "OPEN" == listOrder[i].orderStatus && $scope.status.open++,
                            "LATE" == listOrder[i].orderStatus && $scope.status.late++
                    ...
                    ordersResource.get($scope.restaurantID,"").success(function(payload) {
                        console.log(payload);
                        $scope.orders = payload.data.orders, $scope.countByStatus($scope.orders)
                    })
                }, $scope.statusFilter = function(status) {
                    globalHandleUI.handleHighlightFitler(status), $scope.status = {
                        open: 0,
                        late: 0
                    };
                    var params = "";
                    status && (params = "?status=" + status),
                        ordersResource.get($scope.restaurantID,params).success(function(payload) {
                            $scope.orders = payload.data.orders, "" === params && $scope.countByStatus($scope.orders)
                        })
            ...
            }, $scope.init(), $scope.handleTableScroll = function() {
            var maxRows = 20,
            ...
            var modalInstance = $modal.open({
                templateUrl: "scripts/owner/order/views/modal/order.html",
                resolve: {
                ...
                size: "lg",
                controller: function($scope, $modalInstance, ordersResource, itemToProcess) {
                    $scope.init = function() {
                        $scope.itemToProcess = angular.copy(itemToProcess) || {}
                    }, $scope.init(),
                        $scope.getTotal = function() {
                            for (var orders = $scope.itemToProcess.orderDetails,
                                     total = 0, i = 0; i < orders.length; i++)
                        ...
                        $scope.tempItem = angular.copy(item), $scope.tempItem.orderStatus = "ACCEPTED", delete $scope.tempItem.customer, delete $scope.tempItem.orderDetails, $scope.acceptLoading = !0, setTimeout(function() {
                            ordersResource.addOrUpdate($scope.tempItem).success(function() {
                                $modalInstance.close()
                            })
                        }, 300)
                ...
                if (data) {
                    for (var i = 0; i < $scope.orders.length; i++)
                        if ($scope.orders[i].id == data.id) {
                            isMatchId = !0, $scope.orders[i] = data, alertify.success("Updated Successfully"), $state.go($state.current, {}, {
                                reload: !0
                            });
                            break
                        }
