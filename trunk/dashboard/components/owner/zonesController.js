angular.module("app.owner.restaurant.zones", ["theme.google_maps"])
    .controller("ZoneController", function($rootScope, $scope, $modal, $state, ngTableConfig, deliveryZoneResource) {
        $scope.initZoneMap = function() {
            var setMarker = function(map, center) {
                ...
                drawZone = function(map, zones) {
                    for (var polygon = {}, i = 0; i < zones.length; i++)
            ...
            deliveryZoneResource.get().success(function(payload) {
                console.log(payload);
                if ($scope.deliveryZone = payload.data.deliveryzones, $scope.tableParams = ngTableConfig.config($scope.deliveryZone), $rootScope.globalRestaurantLocation) {
                    $scope.mapCenter = "object" != typeof $rootScope.globalRestaurantLocation ?
                        JSON.parse($rootScope.globalRestaurantLocation) : $rootScope.globalRestaurantLocation, $scope.zoneMap = new GMaps({
                        div: "#zone-map",
                        zoom: 12,
            ...
            item ? (event.stopPropagation(), item.loading = !0) : $scope.createLoading = !0,
                setTimeout(function() {
                    var modalInstance = $modal.open({
                        templateUrl: "scripts/owner/restaurant/views/modal/zone.html",
                        resolve: {
                        ...
                        controller: function($scope, $modalInstance, itemToProcess, deliveryZoneResource, scope) {
                            $scope.itemToProcess = angular.copy(itemToProcess) || {}, $scope.drawOnMap = {
                                ...
                                $scope.saveLoading = !0, $scope.getCoordinates(), $scope.itemToProcess.id || ($scope.itemToProcess.restaurantId = $rootScope.restaurantID), setTimeout(function() {
                                    deliveryZoneResource.addOrUpdate($scope.itemToProcess).success(function() {
                                ...
                                for (var i = ($scope.coordinates.length, 0); i < $scope.coordinates.length; i++) {
                                    ...
                                    tempObj.lat = $scope.coordinates[i].lat(),
                                        tempObj.lng = $scope.coordinates[i].lng(),
                                        $scope.itemToProcess.deliveryZoneCoords.push(tempObj)
                                }
            ...
            event.stopPropagation(), alertify.confirm("Are you sure you want to delete?", function(e) {
                e && deliveryZoneResource["delete"](item).success(function() {
